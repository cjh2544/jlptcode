...
const MYPAGE_CARDS = [
  {
    key: "home.mypageWords",
    descKey: "home.mypageWordsDesc",
    href: "/mypage/words",
    icon: "fas fa-bookmark",
    gradient: "from-violet-500 to-purple-600",
  },
  {
    key: "home.mypageProgress",
    descKey: "home.mypageProgressDesc",
    href: "/mypage/progress",
    icon: "fas fa-chart-bar",
    gradient: "from-amber-500 to-orange-600",
  },
  {
    key: "home.mypageAchievement",
    descKey: "home.mypageAchievementDesc",
      ...
      <section className="relative z-20 app-page-bg -mt-24 pb-20 pt-28">
        <div className="container mx-auto px-4">
          ...
          <div className="home-mypage">
            <div className="home-mypage-panel">
              <div className="home-mypage-head">
                <div>
                  <h2>{t("home.mypageTitle")}</h2>
                  <p>{t("home.mypageHint")}</p>
                </div>
                {isLoggedIn ? (
                  <Button asChild>
                    <Link href="/mypage/words">{t("home.mypageCta")}</Link>
                  </Button>
                ) : (
                  <Button onClick={() => signIn()}>{t("home.mypageSignIn")}</Button>
                )}
              </div>
              <div className="home-mypage-links">
                {MYPAGE_CARDS.map((card) => (
                  <Link
                    key={card.key}
                    scroll={false}
                    href={card.href}
                    className="home-mypage-link"
                  >
                    <span
                      className={`home-card-icon bg-linear-to-br ${card.gradient}`}
                      aria-hidden
                    >
                      <i className={`${card.icon} text-lg`} />
                    </span>
                    <span className="home-mypage-link-copy">
                      <span className="home-mypage-link-title">{t(card.key)}</span>
                      <span className="home-mypage-link-desc">{t(card.descKey)}</span>
                    </span>
                  </Link>
                ))}
              </div>
