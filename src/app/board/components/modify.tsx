...
const BoardWrite = (props: BoardWriteProps) => {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if(procType === 'delete') {
      if(!confirm(t('board.deleteConfirm'))) return false;
    }
                  ...
                  <form onSubmit={onSubmit} className="space-y-4 md:space-y-6">
                      <div>
                          ...
                          <input
                            ...
                            required={true} maxLength={100} type="text" name="title" id="title" className="border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={t('board.placeholderTitle')} />
                      ...
                      <div className='flex justify-center gap-2'>
                        {isLoading ? (
                          ...
                          <>
                            {isMyWrite() && (
                              <>
                                <button onClick={() => setProcType('update')} type="submit" className={`hover:bg-blue-700 bg-blue-500 text-white font-bold py-2 px-4 rounded focus:outline-hidden`}>
                                  {t('board.update')}
                                </button>
                                <button onClick={() => setProcType('delete')} type="submit" className={`hover:bg-red-700 bg-red-500 text-white font-bold py-2 px-4 rounded focus:outline-hidden`}>
                                  {t('board.delete')}
                                </button>
                              </>
                            )}
