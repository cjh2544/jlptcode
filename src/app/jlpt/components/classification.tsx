          ...
          <div key={`class-${cIdx}`}>
            ...
            <div className="grid grid-cols-4 gap-4 sm:grid-cols-2">
              {classificationInfo.years.map((year: string, yIdx: number) => {
                return (
                  <button key={`btn-${cIdx}-${yIdx}`} onClick={handleClick(classificationInfo.classification, year)} className="bg-lightBlue-500 text-white active:bg-lightBlue-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                    {`${year}`}
                  </button>
                )
              })}
