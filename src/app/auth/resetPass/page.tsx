            ...
            <div className="w-full bg-white rounded-lg shadow-lg sm:max-w-md xl:p-0">
              <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                  ...
                  <form onSubmit={onSubmit} className="space-y-4 md:space-y-6">
                    <div>
                        <label className={`block mb-2 text-sm font-bold`}>{t('auth.email')}</label>
                        <div className="flex gap-1">
                          ...
                          <button type="button" className="w-3/6 bg-green-500 hover:bg-green-700 text-white font-bold rounded focus:outline-hidden">
                            {t('auth.sendCode')}
                          </button>
                        </div>
                    </div>
                    <div>
                        <label className={`block mb-2 text-sm font-bold`}>{t('auth.verifyCode')}</label>
                    ...
                    {isLoading ? (
                      <>
                        <button disabled type="submit" className="cursor-not-allowed w-full bg-blue-300 text-white font-bold py-2 px-4 rounded focus:outline-hidden">
                      ...
                      <div className="flex gap-1 text-center">
                        <button type="button" className="w-1/2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-hidden">
                          {t('auth.signIn')}
                        </button>
                        <Link href="/auth/signin" className="w-1/2 bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded focus:outline-hidden">
                          {t('auth.cancel')}
                        </Link>
                      </div>
                    )}
